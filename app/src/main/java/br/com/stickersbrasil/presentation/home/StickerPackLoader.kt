...
import br.com.stickersbrasil.BuildConfig
import br.com.stickersbrasil.data.type.Sticker
...
internal object StickerPackLoader {
    ...
    private fun getStickersForPack(context: Context, stickerPack: StickerPack): List<Sticker> {
        val stickers =
            fetchFromContentProviderForStickers(
    ...
    private fun fetchFromContentProvider(cursor: Cursor): ArrayList<StickerPack> {
        ...
        do {
            ...
            val androidPlayStoreLink = cursor.getString(cursor.getColumnIndexOrThrow(
                StickerContentProvider.ANDROID_APP_DOWNLOAD_LINK_IN_QUERY))
            val iosAppLink = cursor.getString(cursor.getColumnIndexOrThrow(StickerContentProvider.IOS_APP_DOWNLOAD_LINK_IN_QUERY))
            val publisherEmail = cursor.getString(cursor.getColumnIndexOrThrow(
                StickerContentProvider.PUBLISHER_EMAIL))
            val publisherWebsite = cursor.getString(cursor.getColumnIndexOrThrow(
                StickerContentProvider.PUBLISHER_WEBSITE))
            val privacyPolicyWebsite = cursor.getString(cursor.getColumnIndexOrThrow(
                StickerContentProvider.PRIVACY_POLICY_WEBSITE))
            val licenseAgreementWebsite = cursor.getString(cursor.getColumnIndexOrThrow(
                StickerContentProvider.LICENSE_AGREENMENT_WEBSITE))
            val imageDataVersion = cursor.getString(cursor.getColumnIndexOrThrow(
                StickerContentProvider.IMAGE_DATA_VERSION))
    ...
    private fun fetchFromContentProviderForStickers(identifier: String, contentResolver: ContentResolver): List<Sticker> {
        ...
        if (cursor != null && cursor.count > 0) {
            ...
            do {
                ...
                val emojisConcatenated = cursor.getString(cursor.getColumnIndexOrThrow(
                    StickerContentProvider.STICKER_FILE_EMOJI_IN_QUERY))
                stickers.add(
                    Sticker(
                        name,
                        Arrays.asList(*emojisConcatenated.split(",").toTypedArray())
                    )
                )
            } while (cursor.moveToNext())
        }
    ...
    private fun getStickerListUri(identifier: String): Uri {
        return Uri.Builder().scheme(ContentResolver.SCHEME_CONTENT).authority(BuildConfig.CONTENT_PROVIDER_AUTHORITY).appendPath(
            StickerContentProvider.STICKERS).appendPath(identifier).build()
    }

    @JvmStatic
    fun getStickerAssetUri(identifier: String?, stickerName: String?): Uri {
        return Uri.Builder().scheme(ContentResolver.SCHEME_CONTENT).authority(BuildConfig.CONTENT_PROVIDER_AUTHORITY).appendPath(
            StickerContentProvider.STICKERS_ASSET).appendPath(identifier).appendPath(stickerName).build()
    }
}